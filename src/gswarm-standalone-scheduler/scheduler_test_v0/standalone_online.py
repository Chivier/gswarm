...
class WorkflowInstance:
    ...
    def get_ready_nodes(self) -> List[str]:
        ...
        return [nid for nid, node in self.nodes.items() if not node.completed and self.is_node_ready(nid)]
...
class BaselineScheduler:
    ...
    def execute(self, workflows: List[WorkflowInstance]) -> Dict:
        ...
        for workflow in workflows:
            # Process workflow arrival
            current_time = max(current_time, workflow.arrival_time)
            ...
            while not workflow.is_completed():
                ...
                if ready_nodes:
                    ...
                    allocated_gpu_ids, wait_time = self.allocate_gpus(node.model, current_time)
                    current_time = wait_time

                    # Load model on all allocated GPUs (forced reload in baseline)
                    load_time = self.models_config[node.model]["load_time_seconds"]
                    for gpu_id in allocated_gpu_ids:
                        gpu = self.gpus[gpu_id]
                        if gpu.current_model != node.model:
...
class LazyScheduler:
    ...
    def find_gpus_with_model(self, model_name: str, required_count: int, current_time: float) -> List[int]:
        ...
        for gpu in self.gpus:
            if gpu.current_model == model_name and gpu.is_available(current_time):
    ...
    def execute(self, workflows: List[WorkflowInstance]) -> Dict:
        ...
        while pending_workflows or active_workflows:
            ...
            for model_name in tasks_by_model:
                ...
                allocated_gpu_ids, start_time, needs_loading = self.allocate_gpus_lazy(model_name, current_time)
...
def create_workflow_instances(
    requests: List[Dict], workflows_config: Dict, start_time: datetime
    ...
    for req in requests:
        ...
        instance = WorkflowInstance(
            request_id=req["request_id"], workflow_id=req["workflow_id"], nodes=nodes, arrival_time=arrival_time
        )
...
def calculate_metrics(results: Dict) -> Dict:
    ...
    return {
        ...
        "load_overhead": total_load_time / (total_compute_time + total_load_time)
        if (total_compute_time + total_load_time) > 0
        else 0,
        "model_switches": results["total_model_switches"],
    }


def run_experiments(gpu_counts: List[int] = [8, 10, 12, 16, 20]):
    """Run experiments with different GPU counts"""
    config, requests = load_config_and_requests()
    models_config = config["models"]
    workflows_config = config["workflows"]

    # Use first request timestamp as start time
    start_time = datetime.fromisoformat(requests[0]["timestamp"])

    results = {"baseline": {}, "lazy": {}, "metrics": {"baseline": {}, "lazy": {}}}

    for n_gpus in gpu_counts:
        print(f"\nRunning experiment with {n_gpus} GPUs...")
...
def plot_results(results: Dict):
    ...
    for bar, speedup in zip(bars, speedups):
        ...
        ax2.text(bar.get_x() + bar.get_width() / 2.0, height, f"{speedup:.2f}x", ha="center", va="bottom")
    ...
    load_reductions = [(baseline_loads[i] - lazy_loads[i]) / baseline_loads[i] * 100 for i in range(len(gpu_counts))]
    ...
    for bar, reduction in zip(bars, load_reductions):
        height = bar.get_height()
        ax4.text(bar.get_x() + bar.get_width() / 2.0, height, f"{reduction:.1f}%", ha="center", va="bottom")
    ...
    ax7.plot(
        gpu_counts, baseline_throughput, "o-", label="Baseline", color=colors["baseline"], linewidth=2, markersize=8
    )
...
def create_detailed_analysis(results: Dict, gpu_counts: List[int]):
    ...
    for mode, marker, color in [("baseline", "o", "#FF6B6B"), ("lazy", "s", "#4ECDC4")]:
        ...
        ax1.plot(
            gpu_counts, efficiencies, marker=marker, label=mode.capitalize(), color=color, linewidth=2, markersize=8
        )
    ...
    ax3.set_title("Latency Distribution Comparison (20 GPUs)")
    workflows_baseline = results["baseline"][20]["workflows"]
    workflows_lazy = results["lazy"][20]["workflows"]
...
def main():
    ...
    results = run_experiments(gpu_counts=[8, 10, 12, 16, 20])
    ...
    summary = {
        "experiment_config": {"total_workflows": 500, "duration_minutes": 20, "gpu_counts": [8, 10, 12, 16, 20]},
        "results_summary": {},
    }

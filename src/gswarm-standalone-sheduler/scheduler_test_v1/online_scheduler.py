...
class OnlineGreedyScheduler:
    ...
    def _find_best_gpu_assignment(self, task: OnlineTask) -> Tuple[int, float]:
        ...
        if model.gpus_required > self.num_gpus:
            self.logger.warning(
                f"Task {task.request_id}_{task.node_id} requires {model.gpus_required} GPUs "
            ...
            for start_idx in range(self.num_gpus - model.gpus_required + 1):
                ...
                if can_use:
                    ...
                    for gpu_id in gpu_set:
                        ...
                        gpu_available_time = max(
                            self.gpu_states[gpu_id].available_at, self.gpu_reserved_until.get(gpu_id, 0)
                        )
    ...
    def _schedule_task_on_gpu(self, task: OnlineTask, gpu_id: int):
        ...
        scheduled = ScheduledTask(
            node=node_exec, gpu_id=gpu_id, start_time=start_time, end_time=end_time, switch_time=switch_time
        )
        ...
        heapq.heappush(
            ...
            (
                end_time,
                "task_complete",
                {"task_id": f"{task.request_id}_{task.node_id}", "gpu_id": gpu_id, "gpus_used": model.gpus_required},
            ),
        )
        ...
        self.logger.debug(
            f"Scheduled {task.request_id}_{task.node_id} on GPU {gpu_id} at time {start_time:.2f}-{end_time:.2f}"
        )
    ...
    def _process_request_arrival(self, request: Dict):
        ...
        self.request_metrics[request_id] = RequestMetrics(
            request_id=request_id, arrival_time=timestamp, node_count=len(workflow["nodes"])
        )
    ...
    def _process_task_completion(self, event_data: Dict):
        ...
        if task_id in self.pending_tasks:
            ...
            for pending_id, pending_task in self.pending_tasks.items():
                if (
                    pending_id not in self.completed_tasks
                    and task_id in pending_task.dependencies
                    and pending_task.dependencies.issubset(self.completed_tasks)
                ):
        ...
        while (self.event_queue or self.ready_queue) and iteration < max_iterations:
            ...
            if not self.event_queue and self.ready_queue:
                ...
                if min_gpu_time > self.current_time and min_gpu_time < float("inf"):
                    ...
                    self.logger.warning(
                        f"Cannot advance time. Current: {self.current_time}, Ready tasks: {len(self.ready_queue)}"
                    )
                    break
            ...
            log_data["executions"].append(
                {
                    ...
                    "waiting_time": self.pending_tasks.get(
                        f"{task.node.request_id}_{task.node.node_id}", OnlineTask("", "", "", "", 0)
                    ).waiting_time,
                }
            )
...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, default=Path("simple_config.json"), help="Path to system configuration file"
    )
    parser.add_argument(
        "--requests", type=Path, default=Path("simple_requests.yaml"), help="Path to workflow requests file"
    )

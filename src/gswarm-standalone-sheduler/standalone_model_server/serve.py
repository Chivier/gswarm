...
def create_serving_instance(model_name: str, devices: Union[str, List[str]]) -> ModelInstance:
    ...
    try:
        ...
        if model_name in state.models_dram:
            ...
            if (
                len(devices) == 1
                and primary_device != dram_device
                and not (hasattr(dram_model.get("model"), "hf_device_map") and dram_model.get("model").hf_device_map)
            ):
                ...
                if instance.model:
                    try:
                        ...
                        if hasattr(instance.model, "hf_device_map") and instance.model.hf_device_map:
                            ...
                            logger.warning(
                                f"Multi-GPU serving requested but model loaded on single device {dram_device}"
                            )
                            ...
                            try:
                                ...
                                raise HTTPException(
                                    status_code=500, detail=f"Cannot move model to {primary_device}: {e}"
                                )
                    except Exception as e:
        ...
        logger.info(
            f"Created serving instance {instance.instance_id} for {model_name} on {device_display}:{instance.port}"
        )
        return instance
...
async def root():
    """Root endpoint"""
    return {"message": "GSwarm Standalone Model Server", "version": "1.0.0", "status": "running"}
...
async def offload_model(request: OffloadRequest):
    ...
    try:
        ...
        return StandardResponse(
            ...
            data={"target": target, "instances_stopped": len(instances_to_remove)},
        )
    ...
    try:
        ...
        if model_type == "llm":
            ...
            if instance.pipeline:
                ...
                output_path = (
                    state.model_cache_dir / f"output_{instance_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                )
    ...
    try:
        ...
        if model_type == "llm":
            ...
            if "height" not in input_data or "width" not in input_data:
                raise HTTPException(status_code=400, detail="Diffusion models require 'height' and 'width' in data")
        ...
        estimated_time = get_estimation_cost(instance.model_name, instance.device, *data_features)

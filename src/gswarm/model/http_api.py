...
app = FastAPI(
    title="GSwarm Model Manager HTTP API",
    description="HTTP API for GSwarm Model Management System",
    version="0.1.0"
)
        ...
        async with state.registry_lock:
            if model_name in state.model_registry:
                return StandardResponse(
                    success=False,
                    message=f"Model {model_name} is already registered"
                )
...
async def add_model_location(model_name: str, device_name: str):
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            message=f"Location {device_name} added for model {model_name}"
        )
...
async def remove_model_location(model_name: str, device_name: str):
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            message=f"Location {device_name} removed for model {model_name}"
        )
...
async def request_model_serving(model_name: str, request: ServeModelRequest):
    ...
    try:
        ...
        return JobResponse(
            success=True,
            message=f"Serve job created for model {model_name}",
            job_id=job_id
        )
...
async def stop_model_service(model_name: str, device_name: str):
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            message=f"Service stopped for model {model_name} on {device_name}"
        )
...
async def create_job_from_yaml(file: UploadFile = File(...)):
    ...
    try:
        ...
        return JobResponse(
            success=True,
            message=f"Job {job_id} created from YAML",
            job_id=job_id
        )

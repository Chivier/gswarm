#!/usr/bin/env python3
"""
Working SGLang PD-Separated Inference Demo
Uses correct arguments for SGLang v0.4.8
"""
...
class SGLangPDServerManager:
    ...
    def launch_prefill_server(self, model: str, port: int, 
        ...
        cmd = [
            ...
            "--chunked-prefill-size", "8192",  # Large chunks for prefill
            "--max-prefill-tokens", "16384",   # Allow large prefills
            "--schedule-policy", "fcfs",       # First-come-first-serve for prefill
        ]
    ...
    def launch_decode_server(self, model: str, port: int,
        ...
        cmd = [
            ...
            "--max-running-requests", "32",    # More concurrent decodes
            "--schedule-policy", "lpm",        # Longest prefix match for decode
            "--stream-interval", "1",          # Faster streaming
        ]
...
async def run_pd_demo(model: str = "microsoft/phi-2"):
    ...
    print("="*70)
    print(f"\nModel: {model}")
    print("Configuration: 2 prefill servers, 3 decode servers")
    print("-"*70)
    
    # Server configuration
    prefill_ports = [30000, 30001]
    decode_ports = [30002, 30003, 30004]
    ...
    try:
        ...
        for port in decode_ports:
            manager.launch_decode_server(model, port, mem_fraction=0.5)
        ...
        test_prompts = [
            ...
            "What are the benefits of exercise? List three:",
        ...
        for i, prompt in enumerate(test_prompts):
            ...
            try:
                result = await process_request_pd_separated(
                    prompt, prefill_url, decode_url, max_tokens=50
                )
        ...
        if results:
            ...
            print(f"  ✓ Independent scaling (2:3 ratio)")
            print(f"  ✓ Better resource utilization")
            print(f"  ✓ {len(prefill_ports)*len(decode_ports)} possible server combinations")
...
def main():
    ...
    parser.add_argument("--simple-test", action="store_true",
                      help="Run simple single-server test")
    ...
    if args.simple_test:
        # Simple test with single server
        print("\nRunning simple test...")
        cmd = [
            sys.executable, "-m", "sglang.launch_server",
        ...
        print("\nLaunch manually and test with:")
        print("curl http://localhost:30000/generate -X POST -H 'Content-Type: application/json' -d '{\"text\": \"Hello\", \"sampling_params\": {\"max_new_tokens\": 10}}'")
    else:
        # Run full PD demo

...
def run_scheduler(scheduler_type, config_file, requests_file, gpus=4, extra_args=None):
    ...
    if scheduler_type == "baseline":
        cmd = ["python", "baseline.py", "--gpus", str(gpus), "--config", config_file, "--requests", requests_file, "--simulate", "true"]
        log_file = "baseline_execution_log.json"
    elif scheduler_type == "offline":
        cmd = ["python", "offline_scheduler.py", "--gpus", str(gpus), "--config", config_file, "--requests", requests_file, "--simulate", "true"]
        log_file = "offline_execution_log.json"
    else:  # online
        cmd = ["python", "online_scheduler.py", "--gpus", str(gpus), "--config", config_file, "--requests", requests_file, "--simulate", "true"]
        if extra_args:
    ...
    try:
        ...
        try:
            with open(log_file, 'r') as f:
                ...
                for key in ["avg_waiting_time", "p99_waiting_time", "avg_response_time", 
                           "p99_response_time", "avg_request_response_time", "p99_request_response_time"]:
...
def compare_metrics(baseline_metrics, offline_metrics, online_metrics):
    ...
    for display_name, key in metrics_info:
        ...
        if isinstance(baseline_val, (int, float)) and isinstance(online_val, (int, float)):
            ...
            print(f"{display_name:<25} {baseline_val:>15.2f} {offline_val if isinstance(offline_val, str) else f'{offline_val:>15.2f}'} "
                  f"{online_val:>15.2f} {improvement_str:>20}")
        else:
            print(f"{display_name:<25} {str(baseline_val):>15} {str(offline_val):>15} "
                  f"{str(online_val):>15} {'N/A':>20}")
    ...
    if isinstance(baseline_metrics.get("avg_waiting_time"), (int, float)) and \
       isinstance(online_metrics.get("avg_waiting_time"), (int, float)):
    ...
    if isinstance(baseline_metrics.get("p99_waiting_time"), (int, float)) and \
       isinstance(online_metrics.get("p99_waiting_time"), (int, float)):

...
def compare_metrics(offline_metrics, online_metrics):
    ...
    for display_name, key in metrics:
        ...
        if isinstance(offline_val, (int, float)) and isinstance(online_val, (int, float)):
            ...
            if key in [
                ...
                print(f"{display_name:<25} {offline_val:>15.2f} {online_val:>15.2f} {diff:>+14.2f} ({pct_diff:+.1f}%)")
            else:
                # Count metrics - show as integers
                print(
                    f"{display_name:<25} {int(offline_val):>15d} {int(online_val):>15d} "
    ...
    # Analyze results
    if isinstance(online_metrics.get("avg_waiting_time"), (int, float)) and isinstance(
        offline_metrics.get("avg_waiting_time"), (int, float)
    ):
        wait_improvement = (
            (offline_metrics["avg_waiting_time"] - online_metrics["avg_waiting_time"])
            / offline_metrics["avg_waiting_time"]
            * 100
        )
    ...
    if isinstance(online_metrics.get("p99_waiting_time"), (int, float)) and isinstance(
        offline_metrics.get("p99_waiting_time"), (int, float)
    ):
        p99_improvement = (
            (offline_metrics["p99_waiting_time"] - online_metrics["p99_waiting_time"])
            / offline_metrics["p99_waiting_time"]
            * 100
        )
    ...
    if isinstance(online_metrics.get("makespan"), (int, float)) and isinstance(
        offline_metrics.get("makespan"), (int, float)
    ):
        makespan_increase = (
            (online_metrics["makespan"] - offline_metrics["makespan"]) / offline_metrics["makespan"] * 100
        )

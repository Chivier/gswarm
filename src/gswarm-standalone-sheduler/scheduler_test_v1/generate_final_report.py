...
with open(report_file, 'w') as f:
    ...
    for config in sorted(df['config'].unique()):
        ...
        for gpu in sorted(config_data['gpu_count'].unique()):
            ...
            if not baseline.empty and not offline.empty:
                ...
                f.write(f"| {gpu:4d} | {b_time:12,.1f} | {o_time:11,.1f} | {speedup:7.2f} | "
        ...
        f.write(f"- Baseline: {baseline_best['gpu_count']} GPUs "
               f"({baseline_best['throughput']:.3f} tasks/s)\n")
        f.write(f"- Offline: {offline_best['gpu_count']} GPUs "
               f"({offline_best['throughput']:.3f} tasks/s)\n\n")
    ...
    for config in sorted(offline_data['config'].unique()):
        ...
        f.write(f"- **{config}**: {avg_switches:.1f} avg switches, "
               f"{switch_overhead:.1f}% overhead\n")
...
for config in df['config'].unique():
    ...
    for scheduler in ['baseline', 'offline']:
        ...
        if not sched_data.empty:
            ...
            ax2.scatter(sched_data['gpu_count'], sched_data['throughput'], 
                       label=f'{scheduler} ({config})', marker=marker, s=50)
...
for config in df['config'].unique():
    ...
    for gpu in sorted(config_data['gpu_count'].unique()):
        ...
        offline = config_data[(config_data['scheduler'] == 'offline') & 
                             (config_data['gpu_count'] == gpu)]
        if not baseline.empty and not offline.empty:
            reduction = ((baseline['makespan'].values[0] - offline['makespan'].values[0]) / 
                        baseline['makespan'].values[0]) * 100
...
for config in reduction_df['config'].unique():
    ...
    ax4.plot(config_data['gpu_count'], config_data['reduction'], 

...
from scheduler_component import (
    ModelInfo, WorkflowNode, GPUState, ScheduledTask
)
...
class StaticScheduler:
    ...
    def __init__(self, gpus: List[int], gpus_per_server: int = 4, 
                 simulate: bool = False, mode: str = "offline"):
        self.gpus = gpus
    ...
    def assign_models_to_gpus(self, analysis: Dict[str, Any]):
        ...
        for server_id in range(num_servers):
            ...
            self.servers[server_id] = ServerInfo(
                server_id=server_id,
                gpu_ids=self.gpus[start_gpu:end_gpu]
            )
    ...
    def _update_ready_nodes(self, completed_node: NodeExecution):
        ...
        request_complete = all(
            self.executions.get(f"{request_id}_{node['id']}", None) and
            self.executions[f"{request_id}_{node['id']}"].status == "completed"
            for node in workflow['nodes']
        )
    ...
    def _process_request(self, request: Dict):
        ...
        self.logger.info(
            f"[{self.current_sim_time:.2f}s] Processing request {request['request_id']} "
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of GPUs per server (default: 4)"
    )
    parser.add_argument(
        "--simulate",
    ...
    parser.add_argument(
        ...
        default="offline",
        help="Scheduling mode (default: offline)"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("simple_config.json"),
        help="System configuration file"
    )
    parser.add_argument(
        "--requests",
        type=Path,
        default=Path("simple_requests.yaml"),
        help="Workflow requests file"
    )

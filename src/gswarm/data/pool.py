...
class DataPoolManager:
                ...
                "chunks": [
                    {
                        ...
                        "size": c.size,
                        "locations": [
                            {"device": loc.device, "path": loc.path, "status": loc.status}
    ...
    async def get_chunk_data(self, chunk_id: str, device: Optional[str] = None) -> bytes:
        ...
        location = None
        if device:
            location = next((loc for loc in chunk.locations 
                           if loc.device == device and loc.status == "available"), None)
        else:
            location = next((loc for loc in chunk.locations 
                           if loc.status == "available"), None)
    ...
    async def move_chunk(
        ...
        if source_device:
            source_loc = next((loc for loc in chunk.locations 
                             if loc.device == source_device), None)
        else:
            source_loc = next((loc for loc in chunk.locations 
                             if loc.status == "available"), None)
        ...
        # Update locations
        chunk.locations.append(DataLocation(
    ...
    def list_chunks(
        ...
        for chunk in self.chunks.values():
            ...
            chunks.append({
                ...
                "locations": [
                    {"device": loc.device, "status": loc.status}

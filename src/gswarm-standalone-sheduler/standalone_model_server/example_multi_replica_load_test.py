...
class ModelServerClient:
    ...
    def download_model(self, model_name: str) -> bool:
        ...
        try:
            response = requests.post(
                f"{self.server_url}/standalone/download", json={"model_name": model_name, "source": "huggingface"}
            )
    ...
    def load_model(self, model_name: str, device: str = None) -> bool:
        ...
        try:
            ...
            response = requests.post(f"{self.server_url}/standalone/load", json=payload)
    ...
    def serve_model(self, model_name: str, device: str) -> Instance:
        ...
        try:
            response = requests.post(
                f"{self.server_url}/standalone/serve", json={"model_name": model_name, "device": device}
            )
    ...
    def call_model(self, instance: Instance, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "instance_id": instance.instance_id}
    ...
    while not client.stop_load_test:
        try:
            ...
            if result["success"]:
                success_count += 1
                print(
                    f"⚡ Worker-{worker_id} Request-{request_count}: {instance.model_name}[{instance.instance_id}] "
...
def main():
    ...
    if "error" not in status:
        ...
        for instance_info in status["serving_instances"]:
            print(
                f"    {instance_info['instance_id']}: {instance_info['model_name']} "

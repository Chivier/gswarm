...
async def collect_and_store_frame():
        ...
        async with state.data_lock:
            ...
            for client_id, client_payload in active_clients_data.items():
                ...
                if client_id not in state.client_gpu_info or not state.client_gpu_info[client_id]:
                    ...
                    continue
                client_hostname = state.client_gpu_info[client_id][0][
                    "hostname"
                ]  # Assuming all GPUs from one client have same hostname
    ...
    if state.output_filename and (
        state.profiling_data_frames or state.gpu_total_util
    ):  # Save if there are frames or accumulated stats
...
async def exit_head_node():
    ...
    return {"message": "Head node exited."}
